...
const PASS_HINT = "Bestanden. Notiere den Namen des Schulhauses. Das ist Passwort 2.";
...
function finalizeCurrentLetter() {
  ...
  if (appMode === "free") {
    ...
    setFeedback("Eingabe läuft. Rückmeldung kommt nach 3 Sekunden Pause.", "neutral");
  }
...
function finishAttemptAfterPause() {
  ...
  recognizedTextEl.textContent = recognized || "∅";
  liveDecode.textContent = recognized || "∅";
  charCountEl.textContent = String(recognized.length);

  if (appMode === "free") {
    statusTextEl.textContent = "Frei gemorst";
    setFeedback(`Die App hat verstanden: ${recognized || "∅"}`, "neutral");
    return;
  }
  ...
  if (appMode === "practice") {
    if (recognized === expectedText) {
      ...
      setFeedback(`Richtig. Die App hat verstanden: ${recognized}.`, "success");
    } else {
      statusTextEl.textContent = "Noch nicht richtig";
      setFeedback(
        `Noch nicht richtig. Die App hat verstanden: ${recognized || "∅"}. Erwartet war: ${expectedText}.`,
        "error"
      );
    }
    return;
  ...
  if (appMode === "exam") {
    if (recognized === expectedText && !longPauseViolation) {
      ...
      setFeedback(`Nicht verstanden, wiederholen. Die App hat verstanden: ${recognized || "∅"}.`, "error");
...
function startApp() {
  ...
  if (appMode === "practice") {
    ...
    hintText.textContent = "Der Satz wird ohne Leerzeichen geprüft: MEINNAMEISTVORNAMENACHNAME.";
